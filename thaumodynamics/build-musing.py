#!/usr/bin/env python3
"""Build THIS musing (Thaumodynamics) — an HTML-first set, copied verbatim.
...
hand-authored, self-contained HTML pages (a gallery ``index.html`` + three content
...
gets:
    ...
    site/musings/<slug>/thaumodynamics.html      <- monograph
    site/musings/<slug>/mdyn101-worksheet3.html  <- worksheet
    site/musings/<slug>/ashfield-bout.html       <- duel chronicle

Only ``*.html`` is copied: ``README.md`` (human doc) and ``THAUMODYNAMICS.md``
...
in ``MUSING-CONFIG.json``, passing ``--out``. Also runnable on its own::
    ...
    python thaumodynamics/build-musing.py            # -> site/musings/<slug>/
    python thaumodynamics/build-musing.py --out DIR
