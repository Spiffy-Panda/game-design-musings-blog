...
public sealed class McpServer
{
    ...
    private static readonly (string Name, string Desc, string Schema)[] Tools =
    {
        ...
        ("click_at", "Click a point; returns the hit-stack + which control consumed it.",
            """{"type":"object","properties":{"x":{"type":"number"},"y":{"type":"number"},"normalized":{"type":"boolean"},"button":{"type":"string","enum":["left","right","middle"]},"report_hits":{"type":"boolean"},"clicks":{"type":"integer","minimum":1,"description":"1 = single, 2 = double-click"}},"required":["x","y"]}"""),
        ...
        ("wait_for", "Wait: {ms} sleep, {settled_ms} until the frame stops changing, or {element_visible handle}.",
            """{"type":"object","properties":{"ms":{"type":"integer"},"settled_ms":{"type":"integer"},"element_visible":{"type":"object","description":"an element handle, e.g. {\"test_id\":\"btn-step\"}"},"timeout_ms":{"type":"integer"}}}"""),
    ...
    private static readonly Dictionary<string, string[]> Accepts = new()
    {
        ...
        ["click_at"] = new[] { "x", "y", "normalized", "button", "report_hits", "clicks" },
        ["click_element"] = HandleKeys.Concat(new[] { "button", "force", "index", "anchor", "clicks" }).ToArray(),
        ...
        ["wait_for"] = new[] { "ms", "settled_ms", "element_visible", "timeout_ms" },
