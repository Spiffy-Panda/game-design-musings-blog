#!/usr/bin/env python3
"""Build THIS musing (Adventuring Guild Teller) — an HTML-first set, copied verbatim.
...
hand-authored, self-contained HTML pages (a hub ``index.html`` + content pages),
each of which opens straight from disk. Publishing = copying them into the site
unchanged:

    site/musings/<slug>/index.html     <- index.html   (the hub)
    site/musings/<slug>/pitch.html     <- the pitch, read back for correction
    site/musings/<slug>/research.html  <- the desk-research precedent scan

Only ``*.html`` is copied: ``README.md`` (human doc) and
``ADVENTURING-GUILD-TELLER.md`` (agent-nav spec) are internal and never reach the
public surface.

Cross-musing note: pages here may link the sibling set with relative hrefs like
...
in ``MUSING-CONFIG.json``, passing ``--out``. Also runnable on its own::
    ...
    python adventuring-guild-teller/build-musing.py            # -> site/musings/<slug>/
    python adventuring-guild-teller/build-musing.py --out DIR
